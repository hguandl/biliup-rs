...
pub async fn upload_by_app(studio_pre: StudioPre, proxy: Option<&str>, user_agent: Option<&str>) -> Result<ResponseData> {
    // let file = std::fs::File::options()
    //     .read(true)
    //     .write(true)
    //     .open(&cookie_file);
    ...
    Ok(bilibili.submit_by_app(&studio, proxy, user_agent).await?)
}